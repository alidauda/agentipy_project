...
class  TrendingTokens(BaseTool):
    name:str = "Trending Tokens"
    description:str = (
        "Get list of trending tokens on Solana"
        "This tool provides a list of the most trending tokens on Solana"
        "based on their market cap and trading volume"
    )
...
class  TokenPriceData(BaseTool):
    name:str = "Token Price Data"
    description:str = (
        "Get price data for a specific token"
        "This tool provides price data for a specific token"
    )
    """
    Inputs:
    - token_address: string, the mint address of the token, e.g., "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN"
    """
    agent: SolanaAgentKit

    async def _run (self, token_address:str) -> str:
        """Get price data for a specific token"""
        try:
            price_data = await CoingeckoManager.get_token_price_data(self.agent, [token_address])
            if token_address in price_data:
